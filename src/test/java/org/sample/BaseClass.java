...
public class BaseClass {
	...
	public void mouseHoverAction(WebElement target) {
		...
		// // Method for keypress & keyRelease
		// private void robotKey(int keyCode) throws AWTException {
		// Robot r = new Robot();
		// r.keyPress(keyCode);
	...
	private void windowHandle(String id) {
		driver.switchTo().window(id);
		driver.getWindowHandle();
	}

	private void windowHandles(String all) {
		driver.getWindowHandles();
		driver.switchTo().window(all);
	}

	// isDisplayed,Selected,Enabled
	private void displayed(WebElement user, boolean b) {
		user.isDisplayed();
	}

	private void selected(WebElement btn, boolean b) {
		btn.isSelected();
	}

	private void enabled(WebElement txtbox, boolean b) {
		txtbox.isEnabled();
	}
